...
export default function EditLaunchProduct({ params }) {
      ...
      try {
        ...
        if (data.success) {
          setFormData({
            ...
            stock_status: data.data.stock_status || "Choose",
            status: data.data.status || "Choose",
            page_design: data.data.page_design || "Choose",
          });
          setCurrentDesktopImages(data.data.desktop_images || []);
          setCurrentMobileImages(data.data.mobile_images || []);
        } else {
  ...
  const removeNewDesktopImage = (index) => setDesktopImages((prev) => prev.filter((_, i) => i !== index));
  const removeNewMobileImage = (index) => setMobileImages((prev) => prev.filter((_, i) => i !== index));
  ...
  return (
    <div className="p-4 sm:p-6 bg-white shadow-sm rounded-lg">
      <div className="mb-6 border-b border-gray-200 pb-3">
        <h1 className="text-2xl font-normal text-gray-800">Edit Launch Product</h1>
      </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {currentDesktopImages.map((url, i) => (
                    <div key={i} className="relative group border rounded p-1 bg-white">
                      <img src={url} alt="Desktop" className="h-16 object-contain" />
                      <button type="button" onClick={() => removeCurrentDesktopImage(url)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition">x</button>
                    </div>
                  ))}
                </div>
              </div>
            )}
            <p className="text-sm text-gray-500 mb-2">Upload new images (Multiple allowed)</p>
                ...
                {desktopImages.map((file, i) => {
                  ...
                  return (
                    <div key={i} className="relative group border rounded p-1 bg-white">
                      ...
                      <button type="button" onClick={() => removeNewDesktopImage(i)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition">x</button>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentMobileImages.map((url, i) => (
                    <div key={i} className="relative group border rounded p-1 bg-white">
                      <img src={url} alt="Mobile" className="h-16 object-contain" />
                      <button type="button" onClick={() => removeCurrentMobileImage(url)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition">x</button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {mobileImages.map((file, i) => {
                  ...
                  return (
                    <div key={i} className="relative group border rounded p-1 bg-white">
                      {objUrl && <img src={objUrl} alt="" className="h-16 object-contain" />}
                      <button type="button" onClick={() => removeNewMobileImage(i)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition">x</button>
                    </div>
                  );
                })}
              </div>
