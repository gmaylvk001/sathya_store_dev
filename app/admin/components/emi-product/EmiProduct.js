...
export default function EmiProduct() {
  ...
  const fetchProducts = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("/api/emi-product");
      const data = await res.json();
      if (data.success && Array.isArray(data.data)) {
  ...
  const handleOpenEdit = (item) => {
    setFormData({
      _id: item._id,
      id: item.id,
      emiFinance: item.emiFinance?._id || "",
  ...
  const handleCreateSubmit = async (e) => {
    e.preventDefault();
    if (!formData.itemCode.trim() || !formData.schemeCode.trim()) {
      alert("Item Code and Scheme Code are required");
      return;
    }

    try {
      setIsSubmitting(true);
      const res = await fetch("/api/emi-product/add", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          emiFinance: formData.emiFinance,
      ...
      if (res.ok && result.success) {
        await fetchProducts();
        setView("list");
        setSuccessMessage("EMI Product Created Successfully");
        setShowSuccessModal(true);
        setTimeout(() => setShowSuccessModal(false), 2000);
      } else {
        alert(result.error || "Failed to create EMI Product");
      }
    } catch (error) {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      setIsSubmitting(true);
      const res = await fetch("/api/emi-product/update", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok && result.success) {
        await fetchProducts();
        setView("list");
        setSuccessMessage("EMI Product Updated Successfully");
        setShowSuccessModal(true);
        setTimeout(() => setShowSuccessModal(false), 2000);
      } else {
        alert(result.error || "Failed to update EMI Product");
      }
    } catch (error) {
  ...
  const executeDelete = async () => {
    ...
    try {
      if (actionToDelete.type === 'single') {
        const res = await fetch("/api/emi-product/delete", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (res.ok && result.success) {
          ...
          setSuccessMessage("EMI Product Deleted Successfully");
          setShowSuccessModal(true);
          fetchProducts();
        } else {
        ...
        const res = await fetch("/api/emi-product/delete-all", {
          method: "POST",
        });
        const result = await res.json();
        if (res.ok && result.success) {
          setSuccessMessage("All EMI Products Deleted Successfully");
          setShowSuccessModal(true);
          fetchProducts();
        } else {
    ...
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-5 mt-5">
        <h2 className="text-3xl font-light text-gray-700">
          {view === "list" ? "EMI Product" : view === "create" ? "Create EMI Product" : "Edit EMI Product"}
        </h2>
      </div>
          ...
          <div className="flex flex-col md:flex-row justify-between mb-5 gap-4">
            ...
            <div className="border border-gray-200 p-4 rounded-lg flex items-center justify-between gap-6 flex-grow max-w-lg">
              <div className="text-sm text-gray-700 space-y-2">
                <div className="font-semibold">Upload Product scheme file:</div>
                <div>
            ...
            <div className="flex-shrink-0 flex items-start">
              ...
              >
                <Icon icon="ic:baseline-add" className="text-lg" /> New EMI Product
              </button>
            </div>
          </div>
          ...
          <table className="w-full border border-gray-200">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-gray-700">
                <th className="p-3 text-left pl-4 font-semibold w-24">ID</th>
                ...
                filteredProducts
                  ...
                  .map((p) => (
                    <tr key={p._id} className="border-b hover:bg-gray-50">
                      <td className="p-3 pl-4">{p.id}</td>
                      <td className="p-3">{p.itemCode}</td>
                      <td className="p-3">{p.schemeCode}</td>
                      <td className="p-3 pr-4">
                        <div className="flex items-center gap-2 justify-end">
                          ...
                          <button
                            onClick={() => {
                              setActionToDelete({ type: 'single', id: p._id });
                              setShowConfirmationModal(true);
                            }}
      ...
      {(view === "create" || view === "edit") && (
        <div className="bg-white shadow-sm border rounded-lg p-6 border-gray-200 max-w-3xl">
          ...
          <form onSubmit={view === "create" ? handleCreateSubmit : handleUpdateSubmit} className="space-y-5">
            ...
            <div className="flex items-center">
              <label className="w-1/4 text-sm font-semibold text-gray-700">EMI Finance</label>
              <select
                className="w-3/4 border rounded p-2 focus:outline-none focus:border-blue-400 bg-white"
                value={formData.emiFinance}
                onChange={(e) => setFormData({ ...formData, emiFinance: e.target.value })}
              >
            ...
            <div className="flex items-center">
              <label className="w-1/4 text-sm font-semibold text-gray-700">Item Code</label>
              <input
                type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete {actionToDelete?.type === 'all' ? "ALL EMI Products" : "this EMI Product"}?
            </p>
