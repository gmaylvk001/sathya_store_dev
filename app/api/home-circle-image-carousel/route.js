...
import { saveCircleImageCarouselImage } from "@/lib/circleImageCarouselUpload";

/**
 * GET /api/home-circle-image-carousel?instanceId= | ?configId=
 */
export async function GET(req) {
  try {
    ...
    let doc = null;
    if (configId) {
      doc = await HomeCircleImageCarousel.findById(configId).lean();
    } else if (instanceId) {
      doc = await HomeCircleImageCarousel.findOne({ instanceId }).lean();
    } else {
...
/**
 * POST /api/home-circle-image-carousel
  ...
  try {
    ...
    for (let i = 0; i < meta.length; i++) {
      ...
      if (file && typeof file === "object" && file.size > 0) {
        const saved = await saveCircleImageCarouselImage(file);
        image = saved.path;
      }
      if (!image) continue;
      const slugVal = String(item.slug || item.url || "").trim();
      items.push({
        image,
    ...
    let doc = await HomeCircleImageCarousel.findOne({ instanceId });
    const payload = {
      name,
      status,
      items,
    ...
    if (doc) {
      await HomeCircleImageCarousel.updateOne({ instanceId }, { $set: payload });
    } else {
      await HomeCircleImageCarousel.create({
        instanceId,
        ...payload,
      });
    }
    ...
    console.error("POST home-circle-image-carousel", err);
    return NextResponse.json(
      { success: false, message: err.message },
      { status: 500 }
    );
...
/**
 * DELETE /api/home-circle-image-carousel?instanceId=
 */
export async function DELETE(req) {
  try {
    ...
    const doc = await HomeCircleImageCarousel.findOneAndDelete({ instanceId });
