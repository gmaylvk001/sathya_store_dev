...
/**
 * GET /api/home-split-banner?instanceId= | ?configId=
 */
export async function GET(req) {
  try {
    ...
    let doc = null;
    if (configId) {
      doc = await HomeSplitBanner.findById(configId).lean();
    } else if (instanceId) {
      doc = await HomeSplitBanner.findOne({ instanceId }).lean();
    } else {
...
export async function POST(req) {
  try {
    ...
    let doc = await HomeSplitBanner.findOne({ instanceId });
    if (doc) {
      await HomeSplitBanner.updateOne({ instanceId }, { $set: payload });
    } else {
      await HomeSplitBanner.create({ instanceId, ...payload });
    }
    ...
    console.error("POST home-split-banner", err);
    return NextResponse.json(
      { success: false, message: err.message },
      { status: 500 }
    );
...
/**
 * DELETE /api/home-split-banner?instanceId=
 */
export async function DELETE(req) {
  try {
    ...
    const doc = await HomeSplitBanner.findOneAndDelete({ instanceId });
