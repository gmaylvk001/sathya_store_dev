...
import { saveCategorySingleBannerProductsImage } from "@/lib/categorySingleBannerProductsUpload";

const MIN_PRODUCTS = 6;

/**
 * GET /api/category-single-banner-products?instanceId= | ?configId=
 */
export async function GET(req) {
  try {
    ...
    let doc = null;
    if (configId) {
      doc = await CategorySingleBannerProducts.findById(configId).lean();
    } else if (instanceId) {
      doc = await CategorySingleBannerProducts.findOne({ instanceId }).lean();
    } else {
...
/**
 * POST multipart — banner + products (min 6)
 */
export async function POST(req) {
  try {
    ...
    if (!Array.isArray(productIds) || productIds.length < MIN_PRODUCTS) {
      return NextResponse.json(
        {
          success: false,
          message: `Add at least ${MIN_PRODUCTS} products`,
        },
        { status: 400 }
      );
    }
    ...
    if (deskFile && typeof deskFile === "object" && deskFile.size > 0) {
      bannerDesktop = await saveCategorySingleBannerProductsImage(deskFile);
    }
    if (mobFile && typeof mobFile === "object" && mobFile.size > 0) {
      bannerMobile = await saveCategorySingleBannerProductsImage(mobFile);
    }
    ...
    let doc = await CategorySingleBannerProducts.findOne({ instanceId });
    if (doc) {
      await CategorySingleBannerProducts.updateOne(
        { instanceId },
        { $set: payload }
      );
    } else {
      await CategorySingleBannerProducts.create({
        instanceId,
        ...payload,
      });
    }
    ...
    console.error("POST category-single-banner-products", err);
    return NextResponse.json(
      { success: false, message: err.message },
      { status: 500 }
    );
...
export async function DELETE(req) {
  try {
    ...
    const doc = await CategorySingleBannerProducts.findOneAndDelete({
      instanceId,
    });
    if (doc) {
