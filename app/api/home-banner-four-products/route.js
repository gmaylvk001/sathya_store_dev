...
/**
 * GET /api/home-banner-four-products?instanceId= | ?configId=
 */
export async function GET(req) {
  try {
    ...
    let doc = null;
    if (configId) {
      doc = await HomeBannerFourProducts.findById(configId).lean();
    } else if (instanceId) {
      doc = await HomeBannerFourProducts.findOne({ instanceId }).lean();
    } else {
...
export async function POST(req) {
  try {
    ...
    let doc = await HomeBannerFourProducts.findOne({ instanceId });
    if (doc) {
      Object.assign(doc, payload);
      await doc.save();
    } else {
      doc = await HomeBannerFourProducts.create({
        instanceId,
        ...payload,
      });
    }
    ...
    console.error("POST home-banner-four-products", err);
    return NextResponse.json(
      { success: false, message: err.message },
      { status: 500 }
    );
...
export async function DELETE(req) {
  try {
    ...
    const doc = await HomeBannerFourProducts.findOneAndDelete({
      instanceId,
    });
    if (doc) {
