...
function mapOrderStatus(value) {
  ...
  if (["pending", "cancelled", "shipped", "Order Placed", "Failure", "payment_initialized"].includes(raw)) {
    return raw;
  }
...
function mapPaymentStatus(value) {
  ...
  if (lower === "payment_initialized") return "payment_initialized";
  return "pending";
}
...
function buildLiveOrder(existOrder, details, user) {
  ...
  return {
    ...
    order_status: mapOrderStatus(existOrder.order_status),
    payment_status: mapPaymentStatus(existOrder.online_pay_ref_status),
