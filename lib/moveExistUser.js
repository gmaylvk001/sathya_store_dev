...
function skipReason(existUser, liveSets, batchSets) {
  ...
  if (liveSets.emailSet.has(email) || batchSets.emails.has(email)) return "duplicate_email";
...
function buildLiveDoc(existUser) {
  const now = new Date();
  const email = String(existUser.email).trim().toLowerCase();
  const mobile = normalizeMobile(existUser.phone);
  ...
  const doc = {
    ...
    mobile,
    email,
    password: existingPassword && isBcryptHash(existingPassword) ? existingPassword : (existingPassword || ""),
  ...
  return {
    doc,
    generatedPassword,
    needsHash: !existingPassword || !isBcryptHash(existingPassword),
    plainPassword: existingPassword && !isBcryptHash(existingPassword) ? existingPassword : generatedPassword,
  };
}
...
export async function processExistUserMoves(existUsers) {
  ...
  for (const existUser of existUsers) {
    ...
    prepared.push(item);
    batchSets.emails.add(item.doc.email);
