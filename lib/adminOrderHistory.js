...
async function sendAdminCancelSms(order, kind, contactNo) {
  try {
...
export async function adminAddOrderHistory({
  ...
  if (pickupType) {
    ...
    if (!api.success) {
      await sendAdminCancelSms(order, "wondersoft_failed", contactNo);
      return {
        success: false,
    ...
    await sendAdminCancelSms(order, "cancelled_wondersoft", contactNo);
  ...
  await sendAdminCancelSms(order, "cancelled_local", contactNo);
