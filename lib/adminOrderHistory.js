...
// SMS held for now — uncomment import + ENABLE_ADMIN_CANCEL_SMS when reconnecting
// import { sendCancelSms as dispatchCancelSms } from "@/lib/cancelSms";

const ENABLE_ADMIN_CANCEL_SMS = false; // set true to send admin cancel SMS again
...
async function notifyAdminCancelSms(order, kind, contactNo) {
  ...
  try {
    ...
    // When reconnecting: import { sendCancelSms as dispatchCancelSms } from "@/lib/cancelSms"
    // return dispatchCancelSms(encodeURIComponent(text), mobile, template.contentid);
    return { success: false, skipped: true, held: true, preview: text };
  } catch (error) {
